...
- [x] 等待新一周 Top 10 Agent 开始跟单。
  ...
  - 回购监控：能力已实现，但当前比赛项目已暂停，云端常驻服务已停。恢复时通过 Base RPC / Flashblocks 监控官方回购地址，并通过白名单 token 的 `> 3000 VIRTUAL` 大额买入 fallback 覆盖回购地址变化。
  - 自动卖出：能力已实现，但当前不在实盘运行。恢复时仍使用真实回购执行地址和窗口机制；命中后卖出热钱包内该代币配置比例。私钥只放本地或服务器 `.env`，不进入文档和代码仓库。2026-04-28 小比例回购触发测试已通过；后续新增执行审计表、receipt follow-up、窗口前健康检查和专项测试。OKX API 只作为第二执行器 roadmap，不作为当前主路径。
