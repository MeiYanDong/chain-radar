# Docs Index

Read order for a new Agent or future review:

1. [Project Map](./project-map.md)
2. [Plan](./plan.md)
3. [Todo](./todo.md)
4. Relevant phase document under [phases/](./phases/)
5. Relevant runbook, research note, or postmortem

`requirements.md` is user-owned product framing. Agents may read and cite it, but should not edit it without explicit authorization.

## Main References

- [Requirements](./requirements.md)
- [Onchain Exit Engine](./onchain-exit-engine/README.md)
- [Agent Scoring Model](./agent-scoring-model.md)
- [Live P&L Price Lag Research](./live-pnl-price-lag-research.md)
- [Auto-Sell VOLTS / ORCL Postmortem](./auto-sell-volts-orcl-postmortem.md)
- [Pot Season 6 Council Report](./pot-season6-council-report.md)
- [Cloud Stability Goal](./cloud-stability-goal-2026-05-04.md)
- [Private Deployment Runbook](./private-deployment-runbook.md)
