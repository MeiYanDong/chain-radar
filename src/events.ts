...
import { FAT_TOKEN, VIRTUAL_TOKEN, FAT_DEPLOY_BLOCK } from './config.js';
...
export async function fetchAllFatTransfers(
  fromBlock: bigint = FAT_DEPLOY_BLOCK
  ...
  console.log(
    `  Scanning blocks ${fromBlock} to ${currentBlock} (${currentBlock - fromBlock} blocks)`
  );
  ...
  console.log(`\r  Progress: 100% — ${events.length} transfers found`);
  events.sort((a, b) =>
